	...
	browser.elementByXPathOrNull(b.Obj.contactslink_xpath, function(err, el2)
{
	...
	console.log('Please enter the contact name');
	c.input(function(cname){/*
	browser.elementByXPath(b.Obj.cfname_xpath, function(err, el4){
	browser.type(el4, cname, function(err) {
	...
	setTimeout(function(){
	var xpath = "//div[contains(text(),'"+cname+"')]";
	console.log(xpath);
	browser.waitForVisibleByXPath(xpath, 60000,function(err){ 
	if(err)
	{
